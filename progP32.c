...
*/
		XferInstruction(NULL,0x3C13FF20);				//lui $s3, 0xFF20
		XferInstruction(NULL,0x3C080000+(i>>16));		//lui $t0,<FLASH_WORD_ADDR(31:16)>
		XferInstruction(NULL,0x35080000+(i&0xFFFF));	//ori $t0,<FLASH_WORD_ADDR(15:0)>
		XferInstruction(NULL,0x8d090000);				//lw $t1, 0($t0)
		XferInstruction(NULL,0xae690000);				//sw $t1, 0($s3)
		XferInstruction(NULL,0);						//nop
		j=0;
		bufferU[j++]=JTAG_SEND_CMD;
